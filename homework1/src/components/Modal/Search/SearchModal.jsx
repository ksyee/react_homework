      ...
      <section className="flex w-full max-w-522pxr flex-col items-center desktop:w-10/12 desktop:max-w-input-desktop">
        ...
        <div className="mt-40pxr flex w-full">
          <div className="hidden tablet:block tablet:flex-1">
            <h3 className="text-[#c4c4c4]">최근 검색어</h3>
            {searchList.length < 1 ? (
              <p className="mt-12pxr text-12pxr text-[#a6a6a6]">
                검색 결과가 없습니다.
              </p>
            ) : (
              searchList.map((item) => {
                return (
                  <ul className="text-[#a6a6a6]" key={item.id}>
                    <li>{item.title}</li>
                  </ul>
                );
              })
          ...
          <div className="tablet:flex-1">
            <h3 className="text-16pxr text-[#c4c4c4]">실시간 인기 검색어</h3>
            <ul className="mt-20pxr flex flex-col gap-8pxr">
              {data
                .sort((a, b) => a.rank - b.rank)
                .map((item) => (
                  <li key={item.rank} className="relative text-12pxr">
            ...
            <time className="mt-16pxr block text-12pxr text-[#6b6b6b]">
              {`${date} ${time}`} 기준
            </time>
          </div>
        </div>
